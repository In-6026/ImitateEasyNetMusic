...
<template>
    <div class="ml-con">
        ...
        <table class="tl-table fillWidth">
            ...
            <tbody>
                <tr
                    v-for="(musicinfo, index) in musicListCtrl.list"
                    @mouseover="showBtnGroup(index)"
                    @mouseleave="hideBtnGroup()"
                    :class="{ topThree: index <= 2, aftThree: index > 2, listBack: !(index % 2) }"
                    :key="musicinfo.id"
                >
                    <td>
                        <div class="hd">
                        ...
                        <div>
                            <div class="flex flex-item-center">
                                <router-link v-if="index <= 2" :to="'/song?id=' + musicinfo.id">
                                    <img
                                        :src="musicinfo?.al?.picUrl + '?param=50y50'"
                                        class="i-pic"
                                        loading="lazy"
                                    />
                                </router-link>
                                ...
                                <router-link
                                    :to="'/song?id=' + musicinfo.id"
                                    class="text-decoration-none music-name"
                                >{{ musicinfo.name }}</router-link>
                            </div>
                        </div>
                    </td>
                    <td class="time-btngrp">
                        <span
                            :class="{ hiden: index === musicListCtrl.hover }"
                        >{{ musicListCtrl.turnDuation(musicinfo.dt) }}</span>
                        <div
                            class="btn-group flex flex-item-center hide"
                            :class="{ hiden: !(index === musicListCtrl.hover) }"
                        >
                            <a class="addto"></a>
                    ...
                    <td class="fillHeight artists">
                        <span v-for="(artist, index) in musicinfo.ar" :key="artist.id">
                            <router-link
                                :to="'/discover/artist?id=' + artist.id"
    ...
    .tl-table {
        ...
        .w4 {
            .wp {
                ...
                background: url("../../../assets/img/table.png") no-repeat 0 -56px;
            }
        }
        .topThree {
            height: 73px;
