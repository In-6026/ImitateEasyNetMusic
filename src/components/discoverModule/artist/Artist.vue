            ...
            <div class="left-warp">
                ...
                <ul class="flex flex-warp">
                    <li>
                        <router-link
                            to="/discover/artist"
                            @click="categoryCtrl.turn(1)"
                            :class="{ active: categoryCtrl.active == 1 }"
                        >推荐歌手</router-link>
                    </li>
                    <li>
                        <router-link
                            to="/discover/artist/signed"
                            @click="categoryCtrl.turn(2)"
                            :class="{ active: categoryCtrl.active == 2 }"
                        >入驻歌手</router-link>
                    </li>
                </ul>
                <div v-for="cat in categoryCtrl.cats" class="blk" :key="cat.name">
                    ...
                    <ul class="flex flex-warp">
                        <li>
                            <router-link
                                @click="categoryCtrl.turn(cat.ids[0])"
                                :class="{ active: categoryCtrl.active == cat.ids[0] }"
                                :to="'/discover/artist/category?id=' + cat.ids[0] + '&type=1&area=' + cat.area"
                            >{{ cat.name }}男歌手</router-link>
                        </li>
                        <li>
                            <router-link
                                @click="categoryCtrl.turn(cat.ids[1])"
                                :class="{ active: categoryCtrl.active == cat.ids[1] }"
                                :to="'/discover/artist/category?id=' + cat.ids[1] + '&type=2&area=' + cat.area"
                            >{{ cat.name }}女歌手</router-link>
                        </li>
                        <li>
                            <router-link
                                @click="categoryCtrl.turn(cat.ids[2])"
                                :class="{ active: categoryCtrl.active == cat.ids[2] }"
                                :to="'/discover/artist/category?id=' + cat.ids[2] + '&type=3&area=' + cat.area"
                            >{{ cat.name }}组合/乐队</router-link>
                        </li>
                    </ul>
                </div>
            </div>
