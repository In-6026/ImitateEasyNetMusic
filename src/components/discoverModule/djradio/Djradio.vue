        ...
        <div class="radio-box relative">
            <ul class="radio-ul fillHeight flex flex-warp">
                <li v-for="catCov in radioCategories.data" :key="catCov.id">
                    <router-link
                        :to="'?id=' + catCov.id"
                        class="fillHeight fillWidth text-decoration-none"
                    >
                        <div
                            class="catCover"
                            :style="'background: url(' + catCov.picWebUrl + ') no-repeat; background-position: 0 0;'"
                        ></div>
                ...
                <div class="u-titl">
                    <h3>
                        <router-link
                            ...
                            class="til-txt text-decoration-none"
                        >推荐节目</router-link>
                    </h3>
                    <router-link
                        to="/discover/djradio/recommend"
                        class="more text-decoration-none"
                    >更多 ></router-link>
                </div>
                <ul class="r-playlist">
                    <li
                        v-for="(program, index) in radioProgram.data"
                        class="fillWidth flex flex-item-center pointer"
                        :class="{ backg: index % 2 == 0 }"
                        @mouseover="radioProgram.enter(index)"
                        @mouseleave="radioProgram.leave()"
                        :key="program.mainSong.id"
                    >
                        <a class="r-pl-cover relative pointer">
                            <img
                                :src="program.coverUrl + '?param=40y40'"
                                class="absolute"
                                width="40"
                                height="40"
                                loading="lazy"
                        ...
                        <div class="cnt">
                            <h3 class="f-thide">
                                <router-link
                                    :to="'/program?id=' + program.mainSong.id"
                                    class="program text-decoration-none"
                                >{{ program.mainSong.name }}</router-link>
                            </h3>
                            <p>
                                <router-link
                                    :to="'/djradio?id=' + program.radio.lastProgramId"
                                    class="detail text-decoration-none"
                                >{{ program.radio.name }}</router-link>
                            </p>
                        </div>
                        <router-link
                            :to="'/discover/djradio/category?id=' + program.radio.id"
                            class="tag"
                        >{{ program.radio.category }}</router-link>
                    </li>
                </ul>
            </div>
            <div class="blk">
                ...
                <ul class="r-playlist">
                    <li
                        v-for="(program, index) in programRank.data"
                        class="fillWidth flex flex-item-center pointer"
                        :class="{ backg: index % 2 == 0 }"
                        @mouseover="programRank.enter(index)"
                        @mouseleave="programRank.leave()"
                        :key="program.program.mainSong.id"
                    >
                        <a class="r-pl-cover relative pointer">
                            <img
                                :src="program.program.coverUrl + '?param=40y40'"
                                class="absolute"
                                width="40"
                                height="40"
                            />
                        ...
                        <div class="cnt">
                            <h3 class="f-thide">
                                <router-link
                                    :to="'/program?id=' + program.program.radio.lastProgramId"
                                    class="program text-decoration-none"
                                >{{ program.program.mainSong.name }}</router-link>
                            </h3>
                            <p>
                                <router-link
                                    :to="'?id=' + program.program.radio.id"
                                    class="detail text-decoration-none"
                                >{{ program.program.radio.name }}</router-link>
                            </p>
                        </div>
                        <router-link
                            :to="'/discover/djradio/category?id=' + program.program.radio.id"
                            class="tag"
                        >{{ program.program.radio.category }}</router-link>
                    </li>
                </ul>
            </div>
        </div>
        ...
        <div v-for="(radio, index) in djRadios.name" class="radimore" :key="radio">
            <div class="u-titl">
                <h3>
                    <span>
                        <router-link
                            :to="'djradio/categroy?id=' + djRadios.type[index]"
                            class="text-decoration-none til-txt"
                        >{{ radio }}</router-link>.&nbsp;电台
                    </span>
                </h3>
                <router-link
                    :to="'/categroy?id=' + djRadios.type[index]"
                    class="more text-decoration-none"
                >更多 ></router-link>
            </div>
