...
const menu = reactive({
    itemPath: ['rec-recommend', 'rec-toplist', 'rec-playlist', 'rec-djradio', 'rec-artist', 'rec-album'],
    itmes: [
        { name: '推荐', href: '' },
    ...
    index: 0,
    initItem() {
        const currentPath =  $route?.matched[2] ? $route?.matched[2].name as string : 'rec-recommend';
            ...
            <ul class="menu-child-list flex flex-item-center">
                <li v-for="(item, index) in menu.itmes" class="fillHeight" :key="item.name">
                    <router-link :to="'/discover' + item.href" class="href">
                        <em
                            @click="menu.changeItem(index)"
                            class="fillHeight text-center"
                            :class="{ menuChildActive: index === menu.index }"
                        >{{ item.name }}</em>
                    </router-link>
                </li>
            </ul>
